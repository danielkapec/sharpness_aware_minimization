...
def train_one_epoch_sam(model, optimizer, dataloader, device, criterion):
    model.train()
    running_loss = 0.0
    running_correct = 0
    total = 0
...
def main(num_epochs: int, num_workers: int):
    ...
    run_dir = Path(f"experiments_sam/run_{timestamp}")
    ...
    scheduler = optim.lr_scheduler.CosineAnnealingLR(
        optimizer.base_optimizer, T_max=num_epochs
    )
